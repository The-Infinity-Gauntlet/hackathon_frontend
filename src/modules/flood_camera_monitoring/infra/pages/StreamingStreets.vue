...
<template>
  <div class="grid justify-center px-5">
    ...
    <div v-else class="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
      <div v-for="cam in orderedCams" :key="cam.id"
        ...
        <div class="overflow-hidden rounded-2xl bg-transparent lg:h-[10vw]">
          <EmbedStreamPlayer v-if="modes[cam.id] === 'embed' && cam.embed_url" :src="cam.embed_url" :title="cam.name"
            class="h-full w-full" />
        ...
        <div class="flex flex-1 flex-col gap-2 px-4">
          <div class="flex items-start justify-between gap-5 py-3 text-sm lg:text-base">
            ...
            <div class="flex shrink-0 gap-1">
              <button type="button"
                class="rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-slate-300 dark:ring-slate-600" :class="modes[cam.id] === 'embed'
                    ? 'bg-emerald-600 text-white ring-emerald-600'
                    : 'bg-transparent text-slate-600 dark:text-slate-300'
                  " :disabled="!cam.embed_url" @click="modes[cam.id] = 'embed'" title="Realtime (Embed)">
                Realtime
              </button>
              <button type="button"
                class="rounded-md px-2 py-1 text-xs font-semibold ring-1 ring-slate-300 dark:ring-slate-600" :class="modes[cam.id] === 'hls'
                    ? 'bg-blue-500 text-white ring-blue-600'
                    : 'bg-transparent text-slate-600 dark:text-slate-300'
                  " @click="modes[cam.id] = 'hls'" title="HLS">
                HLS
              </button>
            </div>
          </div>
          ...
          <div class="flex items-center justify-center gap-1.5">
            <p class="flex items-center gap-2 text-right text-xs lg:text-sm">
              ...
              <span class="text-2xl font-semibold" :class="displayFloodPercent(cam) <= 40
                  ? 'text-[#27CA2C]'
          ...
          <p>
            Status:
            <span :class="cam.status.toLowerCase() === 'online'
                ? 'text-[#27CA2C]'
          ...
          <div class="mt-auto flex justify-center">
            <RouterLink :to="{ path: `/cameras/${cam.id}` }"
