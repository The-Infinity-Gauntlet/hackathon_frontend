                ...
                if (demoHlsUrl) {
                    ...
                    if (demoPred) {
                        const demoPrediction: PredictionData = {
                            ...
                            probabilities: {
                                normal:
                                    demoPred.probabilities?.normal ??
                                    1 - (demoPred.confidence ?? 0) / 100,
                                flooded:
                                    demoPred.probabilities?.flooded ??
                                    (demoPred.confidence ?? 0) / 100,
                            },
                        }
            ...
            return {
                ...
                flood_percentage:
                    c.id === 'demo' && prediction
                        ? Number(
                              Math.min(100, Math.max(0, prediction.probabilities.flooded)).toFixed(
                                  2,
                              ),
                          )
                        : c.flood_percentage,
